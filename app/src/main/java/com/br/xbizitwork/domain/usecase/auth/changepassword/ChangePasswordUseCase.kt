 ...
 * - Success: quando a alteração é realizada com sucesso
 * - Error: quando ocorre erro de negócio ou técnico
 *
 * IMPORTANTE:
 * - Validação de campos ocorre em ValidateChangePasswordUseCase
 * - Validação de credenciais (senha atual) ocorre APENAS no backend
 * - Este UseCase NÃO deve conter lógica de validação
 */
 ...
 *
 * Esta classe executa a lógica de alteração utilizando o [UserAuthRepository],
...
) : ChangePasswordUseCase, FlowUseCase<ChangePasswordUseCase.Parameters, SignUpResult>() {

    override suspend fun executeTask(parameters: ChangePasswordUseCase.Parameters): UiState<SignUpResult> {
        return try {
