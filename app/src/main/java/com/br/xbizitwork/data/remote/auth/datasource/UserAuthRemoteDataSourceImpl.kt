...
import com.br.xbizitwork.data.mappers.toChangePasswordResponseModel
...
class UserAuthRemoteDataSourceImpl @Inject constructor(
    ...
    override suspend fun changePassword(changePasswordRequestModel: ChangePasswordRequestModel): DefaultResult<ChangePasswordResponseModel> {
        return try {
            ...
            if (response.isSuccessful) {
                val result = response.toChangePasswordResponseModel()
                DefaultResult.Success(result)
            } else {
                DefaultResult.Error(message = response.message)
            }
